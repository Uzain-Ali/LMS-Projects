...
class Student:
    def __init__(self, root):
        ...
        headinglbl = Label(root, text="Personal Information Show", font=("arial", 24, "bold"), bg='wheat', fg='red')
        ...
        # Labels of frame-1
        reg_lbl = Label(entry_frame, text="Type your Information here", font=("arial", 16, "bold"), bg='wheat', fg='red')
        ...
        def add_data():
            if self.roll_no.get() == "" or self.name.get() == "" or self.father_name.get() == "" or self.contact.get() == "" or self.add_txt.get(
                ...
                con = pymysql.connect(host='localhost', user='root', password='uzain', database='student_management_system')
                cur = con.cursor()
        ...
        btn_frame.place(x=60, y=590, width=200, height=100)


        update_btn = Button(btn_frame, text='Update', font=("", 12), command=update_data, width="7")
        update_btn.grid(row=10, column=2, padx=10, pady=10)
        ...
        view_frame.place(x=20, y=20, width=1080, height=100)
