...
r = tk.Tk()
r.title("Show Employee's Feedback")
r.geometry("600x300")
...
img = ImageTk.PhotoImage(Image.open("rfbb.jpg"))
...
s.configure("Treeview.Heading", foreground='#36013F', font=('Helvetic', 11))
...
connect = mysql.connector.connect(host="localhost", user="root", password="uzain", database="rms")
conn = connect.cursor()
conn.execute("SELECT * FROM `emp_feed`")
i = 0
for ro in conn:
