...
Label(r, text="Enter Sergeant ID :", font="Times 12 bold", bg="#456179").place(x=200, y=250)
global e1
...
def ok():
    ...
    id = e1.get()
    connect = mysql.connector.connect(host="localhost", user="root", password="uzain", database="vms")
    conn = connect.cursor()
    conn.execute("select * from case_info where ser_id =" + id + "")
...
def okk():
    ...
    #conn.execute("SELECT SUM(taka) FROM case_info WHERE ser_id="+ id +"")
    #conn.execute("SELECT taka from case_info WHERE ser_id =" + id + "")
    conn.execute("SELECT SUM(taka) *5 / (100) as per FROM case_info WHERE ser_id ="+ id+"")
    z=conn.fetchall()
    for i, in z:
         print(i, end="")
    print(type(i))
    ...
    print(t)
    print(type(t))
    Label(r,text="Sergeant 5% Revenue :"+str(t)+"", font="Times 23 bold", bg="#456179").place(x=950, y=240)

e5=Label(r, font="Times 12 bold", bg="#456179").place(x=500, y=350)
Button(r, text="Parsentage", command=okk, height=2, width=10, font="Times 12 bold").place(x=820, y=240)
